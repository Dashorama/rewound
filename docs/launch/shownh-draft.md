...
**Title:** Show HN: agentgrep — grep for everything your AI coding agent ever did
...
agentgrep indexes every session transcript into a local SQLite/FTS5 database and gives you three ways to search it:

- A CLI (`agentgrep search "fts5 trigger bug"` — ripgrep-style output, resume hint included)
...
- A local web UI (`agentgrep serve`) — phone-friendly, server-rendered, no build step. I bind it to my tailnet and search my transcript history from my phone

Everything is local — no telemetry, no account, no network calls. It reads `~/.claude/projects` read-only and writes its own DB to `~/.agentgrep/`. Indexing is incremental (append-only JSONL, so re-indexing after a session is milliseconds), and it keeps history around ("archive mode") even after the source transcript gets pruned.
